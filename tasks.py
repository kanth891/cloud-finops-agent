...
class IdleKillerGrader(TaskGrader):
    ...
    def grade(self, final_state: CloudState, episode_result: EpisodeResult) -> float:
        ...
        max_savings = 70.0
        savings_ratio = min(0.99, episode_result.total_savings / max_savings) if max_savings > 0 else 0.01
        score = savings_ratio
...
class RightsizerGrader(TaskGrader):
    ...
    def grade(self, final_state: CloudState, episode_result: EpisodeResult) -> float:
        ...
        ideal_steps = 4
        efficiency = max(0.01, 1.0 - (episode_result.steps - ideal_steps) / 20.0)
...
class MinefieldGrader(TaskGrader):
    ...
    def grade(self, final_state: CloudState, episode_result: EpisodeResult) -> float:
        ...
        ideal_steps = 8
        efficiency = max(0.01, 1.0 - (episode_result.steps - ideal_steps) / 30.0)

        # Production safety: binary — all or nothing
        production_safety = 0.99 if episode_result.production_violations == 0 else 0.01
